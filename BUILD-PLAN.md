...
## PHASE 6 — Credential control: platform-managed vs tenant-managed integrations
**Objective:** a super-admin switch deciding, per integration (payments/sms/whatsapp/email/storage), whether all gyms use GymHere's shared credentials or each connects its own; tenant-mode gyms get an Integrations page to add/verify/rotate keys. No service code reads env keys directly.

**Build:** `integration_policies` (gym_id nullable override, service, mode, allow_platform_fallback) + `tenant_credentials` (gym_id, service, AES-256-GCM encrypted payload, key_hint, status). `lib/crypto.ts` (AES-256-GCM, CREDENTIALS_ENCRYPTION_KEY). `lib/credentials/resolver.ts` = single source of truth (`resolveCredentials`, `getPlatformPaymentContext`, `CredentialsMissingError`) and the ONLY place that reads env service keys — enforced by a custom ESLint rule. Adapters (Razorpay, Resend) take injected creds. Super admin `/sa/integrations` (per-service platform↔tenant toggle + fallback + impact counts + force-revert). Gym `/app/settings/integrations` (tier-gated `byo_credentials`, provider forms, encrypt-on-save, Test connection, verified/failed, rotate/delete). Platform SaaS billing always uses platform keys. Tenant webhook `/api/webhooks/razorpay/[gymId]` verifies with the gym's secret. Graceful gating (setup banner when tenant-mode + no verified creds). Seed: policies default platform; one demo gym with dummy unverified creds.

**DoD:** flip payments→tenant → gym without creds sees setup banner → add keys + Test → verified → member payment uses gym's key; SaaS checkout still platform keys; gym webhook rejects another gym's signature + idempotent; DB shows ciphertext only, UI masked hints, changes audit-logged; lint proves zero direct env service-key reads outside resolver; force-revert works with no redeploy.
